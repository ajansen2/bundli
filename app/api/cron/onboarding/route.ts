 ...
 *   3: Set up multi-packs
 *   5: Check your bundle analytics
...
export async function GET(request: NextRequest) {
  ...
  for (const store of stores) {
    ...
    if (emailsSent === 0 && daysSinceInstall >= 0) {
      ...
      emailToSend = createBundleEmail(storeName, shopDomain);
    } else if (emailsSent === 2 && daysSinceInstall >= 3) {
      emailToSend = multiPackEmail(storeName, shopDomain);
    } else if (emailsSent === 3 && daysSinceInstall >= 5) {
      emailToSend = analyticsEmail(storeName, shopDomain);
